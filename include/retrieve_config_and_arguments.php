...
$config_file_namepart = 'yakpro-mtm.cnf';    if (($x = getenv('YAKPRO_MTM_CONFIG_FILENAME'))!==false) $config_file_namepart = $x;

                                                            $t_where    = array();
if ($argument_config_filename!='')                          $t_where[]  = $argument_config_filename;                        // --config-file argument
if (($x = getenv('YAKPRO_MTM_CONFIG_FILE'))     !==false)   $t_where[]  = $x;                                               // YAKPRO_MTM_CONFIG_FILE
if (($x = getenv('YAKPRO_MTM_CONFIG_DIRECTORY'))!==false)   $t_where[]  = "$x/$config_file_namepart";                       // YAKPRO_MTM_CONFIG_DIRECTORY
...
if ( $x                  !==false)                          $t_where[]  = "$x/config/$config_file_namepart";                // HOME/config
                                                            $t_where[]  = "/usr/local/YAK/yakpro-mtm/$config_file_namepart"; // /usr/local/YAK/yakpro-mtm
                                                            $t_where[]  = "$yakpro_po_dirname/yakpro-mtm.cnf";               // source_code_directory/default_conf_filename
...
if ( isset($pos) && ($pos!==false) && isset($t_args[$pos+1]) )
{
    $conf->default_link_name = str_replace('$','',$t_args[$pos+1]);     // in the case where the user wrongly prepends a $ sign to the variable name.
    array_splice($t_args,$pos,2);           // remove the 2 args and reorder
}
$pos = array_search('--no-default-link-name',$t_args);    if (isset($pos) && ($pos!==false)) { $conf->default_link_name     = null; array_splice($t_args,$pos,1); }


$pos = array_search('--indent-mode',$t_args);
if ( isset($pos) && ($pos!==false) && isset($t_args[$pos+1]) )
{
    switch(strtolower($t_args[$pos+1]))
    {
        case 'yak pro':
        case 'yakpro':
            $conf->prettyPrinter = 'YAK Pro';
            break;
        case 'standard':
        default:
            $conf->prettyPrinter = 'Standard';
    }
    array_splice($t_args,$pos,2);           // remove the 2 args and reorder
}
